...
# from dataset_i3dfeature import VideoAudioDataset
...
train_dataloader = load_data(data_root, 'train.json', 2)
...
def eval_net(net, loader, device):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for batch in loader:
            ...
            with torch.no_grad():
                all_out = net(input_v, input_a, input_t)
...
for epoch in range(2000):  # loop over the dataset multiple times
    ...
    for idx, i in enumerate(train_dataloader):
        ...
        optimizer.zero_grad()
        all_out = model(input_v, input_a, input_t)
        # vat_out = (c_out * t_out)
        ...
        if idx % 10 == 9:  # print every * mini-batches
            ...
            time_now = time.time()
            print('[%d, %5d] loss: %.3f, vat_loss: %.3f,  cost_time: %.3f' %
                  (epoch + 1, idx + 1, running_loss / 10, running_loss_vat / 10,  cost_time))
