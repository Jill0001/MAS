...
class VideoAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_path = os.path.join(self.root, 'videos', sample_data['video_path']+'.npy')
        ...
        video_npy = torch.Tensor(np.load(video_path)).squeeze()
        #padding video feature
        if video_npy.shape[0]<56:
            zeros4pad = torch.zeros((56-video_npy.shape[0],1024))
            video_npy=torch.cat((video_npy,zeros4pad))
