...
def load_flow_frames(image_dir, vid, start, num):
  frames = []
  for i in range(start, start+num):
    print(os.path.join(image_dir, vid, str(i)+'x.jpg'))
    exit()
    # imgx = cv2.imread(os.path.join(image_dir, vid, vid+'-'+str(i).zfill(6)+'x.jpg'), cv2.IMREAD_GRAYSCALE)
    # imgy = cv2.imread(os.path.join(image_dir, vid, vid+'-'+str(i).zfill(6)+'y.jpg'), cv2.IMREAD_GRAYSCALE)

    imgx = cv2.imread(os.path.join(image_dir, vid, str(i)+'x.jpg'), cv2.IMREAD_GRAYSCALE)
    imgy = cv2.imread(os.path.join(image_dir, vid, str(i)+'x.jpg'), cv2.IMREAD_GRAYSCALE)
...
class Charades(data_utl.Dataset):
    ...
    def __init__(self, split_file, split, root, mode, transforms=None, num=0):
...
#     c = Charades("/home/jiamengzhao/PycharmProjects/video/pytorch-i3d/testinput/test1.avi",)